...
namespace lab3 {
  ...
  const string Actor::turn_start() {
    ostringstream ss;

    // the list of debuffs to delete at the start of the turn
    list<Buff*> buffs_to_be_deleted;


    // loop through all buffs. See what they do on turn start
    for(auto& buff: _buffs) {
      // do the action
      buff.second->on_turn_start();

      // if the duration has run out
      if(buff.second->get_duration() == 0) {
        buffs_to_be_deleted.push_back(buff.second);
        ss << buff.second->get_name() << " fades from " << _name << endl;
      }
    }

    // delete and remove the buffs
    for(Buff* buff : buffs_to_be_deleted) {
      _buffs.erase(buff->get_name());
      delete buff;
    }
  ...
  const string Actor::turn_end() {
    ostringstream ss;

    list<Debuff*> debuffs_to_be_deleted;

    // loop through all debuffs. See what they do on turn start
    for(auto& debuff: _debuffs) {
      // do the action
      debuff.second->on_turn_start();

      // if the duration has run out
      if(debuff.second->get_duration() == 0) {
        debuffs_to_be_deleted.push_back(debuff.second);
        ss << debuff.second->get_name() << " fades from " << _name << endl;
      }
    }

    // delete and remove the buffs
    for(Debuff* debuff : debuffs_to_be_deleted) {
      _debuffs.erase(debuff->get_name());
      delete debuff;
    }
